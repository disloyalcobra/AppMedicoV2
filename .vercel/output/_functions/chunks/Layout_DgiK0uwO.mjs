...
import 'piccolore';
import { B as maybeRenderHead, a3 as addAttribute, b7 as unescapeHTML, Q as renderTemplate, b9 as renderHead, ba as renderSlot } from './sequence_D80S23pC.mjs';
...
const $$SideBar = createComponent(($$result, $$props, $$slots) => {
  ...
  const links = [
    // Dashboard principal - El Staff y Estudiante tienen sus propias redirecciones en index.astro
    { href: "/", label: "Dashboard", icon: ICONS.dashboard, roles: [ROLES.ADMINISTRADOR, ROLES.JEFE_MEDICO, ROLES.DOCTOR, ROLES.NUTRIOLOGO, ROLES.ENTRENADOR] },
    // Link especial para el Estudiante (Acceso directo a su perfil)
    { href: "/mi-perfil", label: "Mi Expediente", icon: ICONS.patients, roles: [ROLES.ESTUDIANTE] },
    // Módulo de Pacientes (Global)
    { href: "/patients", label: "Pacientes", icon: ICONS.patients, roles: [ROLES.DOCTOR, ROLES.JEFE_MEDICO, ROLES.ENTRENADOR, ROLES.ADMINISTRADOR] },
    { href: "/appointments", label: "Agenda", icon: ICONS.appointments, roles: [ROLES.DOCTOR, ROLES.JEFE_MEDICO, ROLES.ADMINISTRADOR, ROLES.NUTRIOLOGO] },
    { href: "/consultations", label: "Consultas", icon: ICONS.medical, roles: [ROLES.DOCTOR, ROLES.JEFE_MEDICO, ROLES.ADMINISTRADOR] },
    { href: "/injuries", label: "Lesiones", icon: ICONS.injuries, roles: [ROLES.DOCTOR, ROLES.JEFE_MEDICO, ROLES.ADMINISTRADOR, ROLES.ENTRENADOR] },
    { href: "/medications", label: "Inventario", icon: ICONS.inventory, roles: [ROLES.STAFF, ROLES.ADMINISTRADOR] },
    { href: "/nutrition/profiles", label: "Nutrición", icon: ICONS.nutrition, roles: [ROLES.NUTRIOLOGO, ROLES.DOCTOR, ROLES.JEFE_MEDICO, ROLES.ADMINISTRADOR, ROLES.ENTRENADOR] },
    { href: "/notes", label: "Alertas y Notas", icon: ICONS.notes, roles: [ROLES.DOCTOR, ROLES.NUTRIOLOGO, ROLES.ENTRENADOR, ROLES.JEFE_MEDICO, ROLES.ADMINISTRADOR] },
    { href: "/reports", label: "Reportes", icon: ICONS.reports, roles: [ROLES.ADMINISTRADOR, ROLES.JEFE_MEDICO] },
    { href: "/users", label: "Usuarios", icon: ICONS.users, roles: [ROLES.JEFE_MEDICO, ROLES.ADMINISTRADOR] }
  ];
  const visibleLinks = links.filter((link) => {
    if (link.href === "/mi-perfil" && Number(user?.roleId) === ROLES.ADMINISTRADOR) return false;
    return checkRole(user, link.roles);
  });
  return renderTemplate`${maybeRenderHead()}<aside class="w-64 glass-strong shadow-2xl flex flex-col min-h-screen border-r border-white/40 z-20 sticky top-0">  <div class="p-6 border-b border-white/30"> <div class="flex items-center gap-3"> <div class="w-10 h-10 bg-gradient-to-br from-teal-500 to-sky-500 rounded-xl flex items-center justify-center shadow-md"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-6 h-6 text-white">${unescapeHTML(ICONS.medical)}</svg> </div> <div> <h1 class="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-teal-700 to-sky-700">MedApp</h1> <p class="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Sistema Médico</p> </div> </div> </div>  <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto scrollbar-hide"> ${visibleLinks.map((link) => renderTemplate`${renderComponent($$result, "SideBarLink", $$SideBarLink, { "href": link.href, "label": link.label, "icon": link.icon, "active": currentPath === link.href || link.href !== "/" && currentPath.startsWith(link.href) })}`)} </nav>  <div class="p-4 border-t border-white/30 mt-auto bg-white/20"> <div class="flex items-center gap-3 px-2 py-2 mb-4"> <div class="w-9 h-9 bg-teal-100 rounded-full flex items-center justify-center border border-teal-200 text-teal-800 font-bold capitalize"> ${user?.firstName?.[0] || "U"} </div> <div class="overflow-hidden"> <p class="text-sm font-semibold text-slate-800 truncate">${user?.firstName} ${user?.lastName}</p> <p class="text-xs text-slate-500 truncate font-medium">Conectado</p> </div> </div> <a href="/api/auth/logout" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-rose-600 hover:bg-rose-50 transition-colors bg-white/40 border border-white/60 shadow-sm font-medium text-sm w-full justify-center"> <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4">${unescapeHTML(ICONS.logout)}</svg> <span>Cerrar Sesión</span> </a> </div> </aside>`;
