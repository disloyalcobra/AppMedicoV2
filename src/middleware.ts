...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const isPublicAuth = path.includes('/auth/activate') || path.includes('/api/auth');
  if (path === '/login' ||
      path === '/register' ||
  ...
  if (path.startsWith('/users')) {
    // Solo Administrador, Jefe Médico y Doctor tienen acceso
    if (![ROLES.JEFE_MEDICO, ROLES.DOCTOR].includes(roleId)) return redirect('/?error=403');
  }
