...
const $$Add = createComponent(async ($$result, $$props, $$slots) => {
  ...
  let patientId = url.searchParams.get("patientId") || "6";
  const apts = await db.execute({
    sql: `SELECT appointmentId FROM Appointments WHERE patientId = ? ORDER BY dateTime DESC LIMIT 1`,
    args: [patientId]
  });
  const mockAptId = apts.rows.length > 0 ? apts.rows[0].appointmentId : 1;
  if (Astro2.request.method === "POST") {
    ...
    await db.execute({
      ...
      args: [mockAptId, diagnosis, symptoms, date]
    });
    return Astro2.redirect(`/patients/${patientId}`);
  }
